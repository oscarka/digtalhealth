...
def load_excel_cases():
    """加载Excel案例数据"""
    try:
        # 读取Excel文件
        excel_data = pd.read_excel("案例健康需求分析9-5(1).xlsx", sheet_name=None)
        ...
        print(f"❌ 加载Excel案例数据失败: {str(e)}")
        return None

def update_ai_service_with_cases():
    """使用Excel案例数据更新AI服务"""
    case_data = load_excel_cases()
    
    if not case_data:
        return False
    
    # 创建更新后的AI服务类
    updated_ai_service_code = f'''
"""
基于Excel案例数据的AI服务更新
"""
...
class AIService:
    ...
    def __init__(self):
        ...
        # Excel案例数据
        self.case_data = {json.dumps(case_data, ensure_ascii=False, indent=2)}
...
'''
    ...
    # 保存更新后的AI服务代码
    with open("app/services/ai_service_updated.py", "w", encoding="utf-8") as f:
        f.write(updated_ai_service_code)
    
    print("✅ AI服务已基于Excel案例数据更新")
    return True

if __name__ == "__main__":
    print("🚀 开始基于Excel案例数据更新AI服务...")
    success = update_ai_service_with_cases()
    if success:
        print("\n🎉 AI服务更新完成！")
        print("📋 更新内容包括：")
        print("  ✅ 基于Excel案例的音频转写示例")
        print("  ✅ 真实的核心信息提取数据")
        print("  ✅ 多维度健康需求评估案例")
        print("  ✅ 基于医学依据的需求验证")
        print("  ✅ 系统性问题解决框架模块")
